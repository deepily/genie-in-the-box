...
class QueryAndResponseTable():
    ...
    #     new_row = [ { "query": query, "query_embedding": query_embedding, "response_raw": response_raw, "response_raw_embedding": response_raw_embedding, "response_conversational": response_conversational, "response_conversational_embedding": response_conversational_embedding, "solution_path_wo_root": "" } ]
    #     self.query_and_response_tbl.add( new_row )
    #     print( f"AFTER: Table.count_rows: {self.query_and_response_tbl.count_rows()}" )
    #
    #     # query_and_response_tbl.create_fts_index( "query" )
    #
    #     querys = [ query ] + [ "what time is it", "well how did I get here" ]
    #     timer = Stopwatch()
    #     for query in querys:
    #         results = self.query_and_response_tbl.search().where( f"query = '{query}'" ).limit( 1 ).select( [ "query", "response_conversational", "response_raw" ] ).to_list()
    #         du.print_banner( f"Synonyms for '{query}': {len( results )} found" )
    #         for result in results:
    #             print( f"Query: [{result[ 'query' ]}], Response: [{result[ 'response_conversational' ]}] Raw: [{result[ 'response_raw' ]}]" )
    #
